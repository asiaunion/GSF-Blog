...
### 도입부 — **맨 앞에 오해 제시 1문장 + 끝에 대표 문장 1문장 추가** (GPT #1 채택)
...
*(변경 근거: 기존 두 문단 그대로, 맨 앞에 오해 제시 1문장 + 끝에 대표 문장 1문장 추가. 새 사실 없음 — "서명이 끝이 아니다"는 §입주기록·§원상회복에서 이미 다루는 내용을 요약한 도입 프레이밍. 기존 대표 문장과 자연스럽게 연결됨)*
...
### H2 "입주 때 왜 기록을 남겨야 하나" — **끝에 노트 문장 1문장 추가** (GPT #2 채택 — 판단 변화형으로 교체)
...
*(변경 근거: 기존 문단 그대로, 이미 승인된 1인칭 실경험을 확장하는 노트 문장 1개만 추가 — 새로운 경험이나 사건을 만들지 않고, 같은 사건에 대한 판단 회고만 덧붙임. GPT 제안대로 "당시엔~나중엔" 판단 변화 구조로 교체 — SSOT "처음 생각 → 데이터/경험 재확인 → 판단 변화" 패턴과 더 정확히 일치)*

### H2 "계약서에서 무엇을 협상해 볼 수 있나" — **변경 없음**
### H2 "서명 전 외국인 체크리스트는" — **변경 없음** (체크리스트 항목 전체)
### H2 "서명 전 한 줄 점검" — **뒤에 여운 있는 마무리 1문장 추가** (GPT #3 채택 — 도입부 오해 제시와 콜백되는 리듬형으로 교체)
...
*(변경 근거: 체크리스트 3항목 원문 그대로. 마무리 문장을 GPT 제안대로 교체 — 기존 "세 가지만 먼저 확인해도" 실용형 요약 대신, 도입부 오해 제시("계약서는 한 번만 보면 끝이라고 생각하기 쉽습니다")와 정확히 대구를 이루는 리듬형 콜백으로 여운을 강화. 새 조언·새 단정 없음)*
...
**"Why record the apartment…" — 노트 문장 추가(판단 변화형)**:
```
...
**"One-line pre-signing check" — 마무리를 도입부 콜백형으로 교체**:
```
...
**도입부 — 오해 제시 + 대표 문장 삽입**:
```
契約書は一度読めば終わりだと思われがちです。しかし実際には、入居後のほうが重要になる場合が少なくありません。
...
> GPT 종합 평가: 9.5/10. "YMYL 안전성 유지하면서 읽기 쉽다"고 평가, 공통 지적은 "발견이 조금 약하다" — 판단이 바뀐 지점을 더 명시적으로 드러내라는 것.

| # | GPT 제안 위치 | GPT 제안 요약 | 채택/기각 | 이유 |
|---|--------------|--------------|-----------|------|
| 1 | 도입부 | 제도 설명으로 바로 들어가지 말고 오해 하나를 먼저 제시 — "계약서는 한 번만 보면 끝이라고 생각하기 쉽습니다. 하지만 실제로는 입주 이후가 더 중요해지는 경우가 많습니다" | **채택** | 기존 대표 문장("낯선 것은 계약서 전체가 아니라…")과 자연스럽게 연결되고, §입주기록·§원상회복에서 이미 다루는 내용의 정확한 예고편 — 새 사실 없음 |
| 2 | §입주기록 노트 문장 | "그 사진 몇 장이 실제로 도움이 될지는 나중에야 알았습니다" → "당시에는 번거로운 작업이라고 생각했지만, 나중에는 가장 시간을 아껴준 기록이 되었습니다"(판단 변화형) | **채택** | GPT 지적대로 "왜 기억에 남았는가"를 보여주는 편이 SSOT "처음 생각→재확인→판단 변화" 패턴에 더 정확히 부합. 같은 사건(입주 첫날 사진 촬영)에 대한 회고이므로 새 경험 아님 |
| 3 | 마무리 | "계약서를 한 번에 다 이해할 필요는 없습니다…" 실용형 요약 → "계약은 서명으로 끝나지만, 실제 생활은 그 이후부터 시작됩니다"(도입부 오해 제시와 대구) | **채택** | 도입부 오해 제시(GPT #1)를 채택하면서 마무리가 그 오해를 정확히 되돌아보는 콜백 구조가 완성됨. 정보 요약형보다 리듬감 있는 여운으로 개선 |
| 4 | 공통 — "발견이 조금 약하다" | 슬러그마다 "처음엔 ○○라고 생각했다/다시 보니 생각이 바뀌었다"류 문장 1~2개 권장 | **반영(위 #1·#2로 충족)** | 도입부 오해 제시 1개 + §입주기록 판단변화 1개로 이미 2개 확보. 추가 삽입은 과잉 개입으로 판단해 보류 |
...
- [x] 대표 문장 1쌍 — 도입("낯선 것은 계약서 전체가 아니라…") ↔ 마무리("계약은 서명으로 끝나지만, 실제 생활은 그 이후부터 시작됩니다") 콜백(GPT #3 반영으로 리듬감 강화)
- [x] 오해 제시 — 도입부("계약서는 한 번만 보면 끝이라고 생각하기 쉽습니다…", GPT #1 신규), 원상회복·외국인보증 2곳(오해→데이터 구조, 총 3회)
- [x] 모바일 3줄 문단 — 신규 삽입 문장 전부 1~2문장, 기존 리듬 유지
- [x] 노트 문장 1회 — 입주 기록 섹션에 판단 변화형("당시에는…나중에는…") 회고 1문장(GPT #2 반영, 새 경험 창작 아님)
- [x] 여운 있는 끝 — 마무리를 도입부 오해 제시와 대구를 이루는 콜백으로 교체(GPT #3). Wave A/B·슬러그1·2와 표현 겹치지 않음
...
- **수정 범위**: 도입부 오해 제시 1문장(GPT #1) + 대표 문장 1문장, §원상회복·§외국인보증 각 오해→데이터 1문장, §입주기록 노트 문장 1문장(GPT #2로 판단변화형 교체), §서명전 한줄점검 뒤 마무리 1문장(GPT #3으로 콜백형 교체). 나머지(협상 섹션·체크리스트 2종·다이어그램·시리즈 링크)는 전부 변경 없음
- **GPT 리뷰**: 9.5/10, 제안 3건 전부 채택 — claude-final로 확정
