# Voice Full B1 Handoff — Ep.1 · Ep.2 · Ep.9

> 대상 3편 (보드 순서 2→3→4):
> 1. `tokyo-core-3-wards-chiyoda-chuo-minato` (Ep.1)
> 2. `tokyo-shinjuku-shibuya-bunkyo` (Ep.2)
> 3. `tokyo-musashino-mitaka-chofu` (Ep.9)
>
> 역할: Claude 주도 · GPT 완성도 리뷰 · AG 반영/1차 검증 · Cursor 최종 검증/배포  
> 전제: #6 파일럿 Joseph 라이브 읽기 통과 (2026-07-18) → B1 해제됨  
> 파이프·HARD 규칙은 파일럿과 동일: `claude-drops/2026-07-18_voice-full-pilot-handoff.md` 준용

## slug별 특이 주의 (파일럿과 다른 부분)

### Ep.1 `tokyo-core-3-wards-chiyoda-chuo-minato` — FA 영향 큼
- Wave B FA에서 구별 소득 수치의 오출처(전국 NTA 통계를 구 단위로 오인용)를 이미 교정/헤지했다. **교정된 서술을 되살리거나 다시 단정형으로 바꾸지 말 것.**
- fact-audit은 Wave B에서 "정직한" 스키마로 재생성됨. 잠금표(A)는 현재 fact-audit 기준으로 작성.

### Ep.2 `tokyo-shinjuku-shibuya-bunkyo` — FA 영향 큼
- Wave B FA에서 단정형 투자 권유 어휘("뿐입니다"·"정답입니다" 계열)를 제거했다. Voice Full에서 **판단 과정을 살리되 투자 권유 톤으로 회귀 금지.**
- Ep.1과 같은 오출처 교정 이력 있음 — 동일 주의.

### Ep.9 `tokyo-musashino-mitaka-chofu` — 구조 복구 직후
- Wave B에서 KO 중복 섹션(미타카·조후 2회 등장) 삭제로 H2 구조를 복구했다. **현재 KO H2 spine이 유일한 기준이며 어떤 이유로도 재변경 금지.**
- 복구 과정에서 문장 흐름이 다소 건조할 수 있음 — Voice Full의 주 타깃은 읽기 리듬 회복.

## 1. Joseph → Claude

```text
GSF-Ark Voice Full B1 배치를 시작한다. #6 파일럿과 동일 파이프다.

대상 slug (이 순서대로, 한 편씩 drop 완성):
1. tokyo-core-3-wards-chiyoda-chuo-minato
2. tokyo-shinjuku-shibuya-bunkyo
3. tokyo-musashino-mitaka-chofu
...
5. claude-drops/2026-07-18_voice-full-b1-handoff.md (이 파일 — slug별 특이 주의)
...
- Wave B FA에서 교정·삭제·헤지된 서술을 되살리지 않는다 (Ep.1·Ep.2 오출처 교정, Ep.2 투자 권유 어휘, Ep.9 구조 복구).
- KO를 기준으로 EN/JA 의미 정합 지시를 함께 낸다.

drop 파일 (slug당 1개):
claude-drops/2026-07-18_voice-full_tokyo-core-3-wards-chiyoda-chuo-minato.md
claude-drops/2026-07-18_voice-full_tokyo-shinjuku-shibuya-bunkyo.md
claude-drops/2026-07-18_voice-full_tokyo-musashino-mitaka-chofu.md
...
8. Wave B FA에서 제거·교정한 claim이 되살아났는가
...
```text
Voice Full B1 <slug> 최종 패키지를 repo에 반영한다.
...
- title/description/slug/H2 spine 변경 금지 (Ep.9는 복구된 현재 spine이 기준).
- KO 기준 EN/JA 의미 정합.
- commit·deploy 금지.
...
- 새 사실/수치/인과/허구 경험 0 · FA 교정 서술 회귀 0.
...
- B1 3편 모두 live 후 B2 해제 여부를 Joseph에게 확인.
