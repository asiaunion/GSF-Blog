...
async function main() {
  ...
  if (args.length < 2) {
    ...
    console.log("Usage: node scripts/translate/translate.mjs <source_file_path> <target_lang> [model_name]");
    console.log("Example: node scripts/translate/translate.mjs src/data/blog/ko/my-post.md ja gemma2:9b");
    process.exit(1);
  }
  ...
  try {
    ...
    const approved = await askApproval("Do you approve and want to save this translation? [Y/n] : ");
